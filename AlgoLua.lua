...
function OnInit()
	...
	trans_id      = ttime;               -- Задает начальный номер ID транзакции
	...
	_Period_ATR = 120;                       -- ATR за 2 последних часа --
		   
	
	_ShortTimeInterval, _LongTimeInterval = 37, 113  -- периоды EMA в секундах
	_MaxQuantityOfPositions              = 10;       -- максимальное количество позиций с которым можно открываться
	...
	ATR_Level = 0.0007 * P -- 70/100'000             -- вычисляет уровень ATR выше которого можно открываться.
	                                                 -- При цене 80000, уровень будет 60.
	...
	data_source = CreateDataSource(class, sec, INTERVAL_M1); -- Создание источника данных по инструменту 'sec'	
	
	last_index = data_source:Size() - 1;

	if ( data_source ~= nil ) -- Подключение источника данных
		then
			message("Источник данных по инструменту " .. sec ..
         			" успешно подключен \n\n" ..
                    "Количество свечей " .. last_index);
            IsRun = true;					
		else 
	...
	sleep(2000); -- Время необходимое для получение данных по инструменту
...
function  main()  
	...
	--------------------- Открытие и закрытие позиций на основании сигналов -------------
	  ...
	  if ( (os.time() > WarmUpTime) and (SessionStatus_2 == 1) and timecond ) then  -- Разогрев Системы 5 минут // Нужно переделать и убрать разогрев!!!
	...
	-- В случае закрывающего ордера, после истечения времени выставляем Market order (KillTransaction)
	...
	----------- Закрываем все открытые позиции перед концом сессий ----------------------
		...
		local cond2 =  nowtime > EVNENDTIME - 60*10; -- Закрываем позиции за 10 минут до конца сессии	
...
function  OnParam(class,sec)
	 ...
	 -- Получение Главных Индикаторов  и Вычисление сигналов открытия и закрытия --
